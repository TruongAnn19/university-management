        ...
        switch (user.getRole()) {
            case STUDENT:
                studentRepository.findByUserId(user.getId())
                        .ifPresent(student -> response.setDetails(profileMapper.toStudentDetails((Student) student)));
                break;
            case TEACHER:
                teacherRepository.findByUserId(user.getId())
                        .ifPresent(teacher -> response.setDetails(profileMapper.toLecturerDetails((Teacher) teacher)));
                break;
            case ADMIN:
                break;
        }
